...
""" Be sure to log into vault before running this script!
    This script will copy all secrets under mount prefixed with 'depricated' 
    and write them to a new mount named the same with no such prefix. 
    For example: deprecated/secret/aws will be copied to secret/aws """
...
def init_destination_mount(source_mount, mount_version):
    ver = mount_version[1]
    destination = source_mount.split("deprecated/")[1]
    print('Going to create new mount:  {} of version {}'.format(destination, ver))
    client.sys.enable_secrets_engine(backend_type='kv', path=destination, options={
        "version": ver
    })
    return destination
...
def main():
    source_mount = input("Enter the mount you wish to Rollback: ")
